"""
SpiceJet Source Adapter (spicejet.com / book.spicejet.com).
Direct airline carrier source for SpiceJet (SG).
...
class SpiceJetSource(BaseFareSource):
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        super().__init__(source_name="spicejet", config=config)
        self.base_url = self.config.get("base_url", "https://book.spicejet.com")
        self.timeout = self.config.get("timeout_seconds", 25)

    def _build_search_url(self, origin: str, destination: str, travel_date: str) -> str:
        """Builds the SpiceJet booking search URL."""
        return f"{self.base_url}/Search.aspx?page=Select&culture=en-GB&market={origin}-{destination}-{travel_date}"
    ...
    def search(
        ...
        """
        Executes search on book.spicejet.com using permitted HTTP requests.
        Detects ASP.NET placeholder shell / WAF blocking and returns SOURCE_BLOCKED.
        """
        ...
        logger.info(f"Initiating SpiceJet search: {origin_code} -> {dest_code} on {travel_date}")
        ...
        try:
            ...
            if response.status_code in (403, 429, 503):
                logger.warning(f"SpiceJet access blocked with status {response.status_code}.")
                return ScrapeResult(
                    ...
                    error_message=f"Access blocked by Cloudflare WAF (HTTP status {response.status_code})"
                )

            raw_html = response.text
            # Check if initial response is an unrendered ASP.NET / JS shell placeholder
            if "Please wait..." in raw_html and not any(k in raw_html for k in ("flight-row", "avail-table", "fare-amount")):
                logger.info("SpiceJet returned unrendered JS app shell without flight schedules.")
                return ScrapeResult(
                    source=self.source_name,
                    origin=origin_code,
                    destination=dest_code,
                    ...
                    error_message="Access requires dynamic JS execution & ASP.NET ViewState (SOURCE_BLOCKED)"
                )
            ...
            logger.warning(f"SpiceJet access blocked/timed out: {err}")
            return ScrapeResult(
                source=self.source_name,
                origin=origin_code,
                destination=dest_code,
                travel_date=travel_date,
                status=CollectionStatus.SOURCE_BLOCKED,
                error_message=f"Access blocked by Cloudflare WAF / Connection error: {err}"
            )
    ...
    def parse_html(
        ...
        """
        Parses SpiceJet flight cards from HTML content into FareObservation objects.
        """
        ...
        cards = soup.find_all(
            "tr",
            class_=lambda c: c and any(k in str(c).lower() for k in ("flight-row", "avail-row", "sg-flight"))
        )
        for card in cards:
            try:
                ...
                logger.debug(f"Error parsing SpiceJet card: {e}")
    ...
    def _parse_single_card(
        ...
        """Parses a single SpiceJet flight card."""
        airline_name = "SpiceJet"
        fl_el = card.find(class_=lambda c: c and ("flight-number" in c or "fl-no" in c))
        raw_fl = fl_el.text.strip() if fl_el else "SG 8169"
