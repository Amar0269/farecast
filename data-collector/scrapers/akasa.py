"""
Akasa Air Source Adapter (akasaair.com).
Direct airline carrier source for Akasa Air (QP).
...
class AkasaSource(BaseFareSource):
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        super().__init__(source_name="akasa", config=config)
        self.base_url = self.config.get("base_url", "https://www.akasaair.com")
        self.timeout = self.config.get("timeout_seconds", 25)

    def _build_search_url(self, origin: str, destination: str, travel_date: str) -> str:
        """Builds the Akasa Air search URL."""
        return f"{self.base_url}/booking/search?src={origin}&dst={destination}&depart={travel_date}&adult=1"
    ...
    def search(
        ...
        """
        Executes search on akasaair.com using permitted HTTP requests.
        Detects connection timeout / WAF blocking and returns SOURCE_BLOCKED.
        """
        ...
        logger.info(f"Initiating Akasa Air search: {origin_code} -> {dest_code} on {travel_date}")
        ...
        try:
            ...
            if response.status_code in (403, 429, 503):
                logger.warning(f"Akasa Air access blocked with status {response.status_code}.")
                return ScrapeResult(
                    ...
                    error_message=f"Access blocked by Akasa WAF (HTTP status {response.status_code})"
                )
            ...
            logger.warning(f"Akasa Air access blocked/timed out: {err}")
            return ScrapeResult(
                source=self.source_name,
                origin=origin_code,
                destination=dest_code,
                travel_date=travel_date,
                status=CollectionStatus.SOURCE_BLOCKED,
                error_message=f"Access blocked by WAF / Connection timeout: {err}"
            )
    ...
    def parse_html(
        ...
        """
        Parses Akasa Air flight cards from HTML content.
        """
        ...
        cards = soup.find_all(
            "div",
            class_=lambda c: c and any(k in str(c).lower() for k in ("flight-card", "qp-flight-row", "bound-card"))
        )
        for card in cards:
            try:
                ...
                logger.debug(f"Error parsing Akasa card: {e}")
    ...
    def _parse_single_card(
        ...
        raw_fl = fl_el.text.strip() if fl_el else "QP 1101"
